...
# Sidebar
st.sidebar.header('CallorNot')
select = st.sidebar.selectbox('', ['Cuestionario'], key='1')
if not st.sidebar.checkbox("Hide", True, key='1'):
    
    st.subheader("Rellena el Cuestionario")
   ...
   #Nª2--------------------------------------
    ...
    # Boton de Prediccion
    submit = st.button('¿ Llamamos o no Llamamos ?')

    if submit:
            prediction = clasificador.predict([[edad, marca_bp, seg_valor, seg_recorrido, marca_ccte,
            ...
            #probabilidades = clasificador.predict_proba([[edad, marca_bp, seg_valor, seg_recorrido, marca_ccte,
             #        camino_digital,saldo_ffii,saldo_plp,lp_dom_ingresos,
              #       lp_rbos,lp_seg_vida,lp_seg_auto,lp_seg_acc,lp_tjta_cto,
               #      lp_tjt_rev,saldo_captacion,saldo_financiacion]])
            ...
            if prediction == 0:
                st.write('No Llames')
                #st.write('Probabilidad de que no contrate: ', probabilidades[:, 0])
            else:
                st.write(" LLAMA ")

else:
    st.write('Por Favor, Habilita el Cuestionario')
